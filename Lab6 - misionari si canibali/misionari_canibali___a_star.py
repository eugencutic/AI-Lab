...
class Graf:
    ...
    def fct_euristica_h2(self, nod_info):
        canibali_est = nod_info[2]
        misionari_est = nod_info[1]
        mal_barca = nod_info[0]
        if (canibali_est + misionari_est != 0 and mal_barca == 'est'):
            return 2 * ( (canibali_est + misionari_est - 2)/(Nod.NR_LOCURI_BARCA - 1) ) + 1
        if (canibali_est + misionari_est != 0 and mal_barca == 'vest'):
            return 2 * ( (canibali_est + misionari_est - 2)/(Nod.NR_LOCURI_BARCA - 1) ) + 2
        return 0


    def muta_barca(self, nod_info_copy, nr_mis, nr_can):
        # in laboratoarele anterioare, am avut aici "interschimba"
        nod_info = nod_info_copy[:]
        mal_barca = nod_info[0]
        if (mal_barca == 'est'):
            nod_info[0] = 'vest'
            nod_info[1] -= nr_mis
    ...
    def calculeaza_succesori(self, nod):
        ...
        for mis in range(min(nr_mis, Nod.NR_LOCURI_BARCA) + 1):
            for can in range(min(nr_can, Nod.NR_LOCURI_BARCA) + 1):
                if (mis >= can and mis+can <= Nod.NR_LOCURI_BARCA and mis+can >= 1):
                    fiu = self.muta_barca(nod.info, mis, can)
                    if ((fiu[1] >= fiu[2] and fiu[3] >= fiu[4]) or fiu[1] == 0 or fiu[3] == 0):
                        l_succesori.append( (Nod(fiu, self.calculeaza_h(fiu)), 1) )
        return l_succesori
