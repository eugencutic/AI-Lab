...
class Graf:
    ...
    def calculeaza_h(self, reprez_nod):
        ...
        return abs((i_scop - i_curent)) + abs((j_scop - j_curent))
    ...
    def calculeaza_succesori(self, nod):
        ...
        succ = []
        poz_curenta = nod.info.index(1)
        i_curent = poz_curenta // 20
        j_curent  = poz_curenta % 20

        if i_curent >= 1:
            poz_new = (i_curent - 1) * 20 + j_curent
            if nod.info[poz_new] != 2:
                reprez = self.interschimba(poz_curenta, poz_new, nod.info)
                h_nod = self.calculeaza_h(reprez)
                succ.append((Nod(reprez, h_nod), 1))
        if i_curent <= 18:
            poz_new = (i_curent + 1) * 20 + j_curent
            if nod.info[poz_new] != 2:
                reprez = self.interschimba(poz_curenta, poz_new, nod.info)
        ...
        if j_curent >= 1:
            poz_new = i_curent * 20 + j_curent - 1
            if nod.info[poz_new] != 2:
                reprez = self.interschimba(poz_curenta, poz_new, nod.info)
                h_nod = self.calculeaza_h(reprez)
                succ.append((Nod(reprez, h_nod), 1))
...
def a_star(graf):
    ...
    f = open('F:/Projects/IA/Lab5 - maze/demo_file.txt', 'a')
